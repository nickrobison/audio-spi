...
public class TCircularBuffer {
    ...
    public int read(byte[] abData, int nOffset, int nLength) {
        if (org.tritonus.share.TDebug.TraceCircularBuffer) {
            org.tritonus.share.TDebug.out(">TCircularBuffer.read(): called.");
            dumpInternalState();
        }
        if (!isOpen()) {
            if (availableRead() > 0) {
                ...
                if (org.tritonus.share.TDebug.TraceCircularBuffer) {
                    org.tritonus.share.TDebug.out("reading rest in closed buffer, length: " + nLength);
                }
            } else {
                if (org.tritonus.share.TDebug.TraceCircularBuffer) {
                    org.tritonus.share.TDebug.out("< not open. returning -1.");
                }
                return -1;
            }
        }
        synchronized (this) {
            if (m_trigger != null && availableRead() < nLength) {
                if (org.tritonus.share.TDebug.TraceCircularBuffer) {
                    org.tritonus.share.TDebug.out("executing trigger.");
                }
                m_trigger.execute();
            }
            if (!m_bBlockingRead) {
            ...
            while (nRemainingBytes > 0) {
                while (availableRead() == 0) {
                    try {
                        ...
                        if (org.tritonus.share.TDebug.TraceAllExceptions) {
                            org.tritonus.share.TDebug.out(e);
                        }
                    }
                }
            ...
            if (org.tritonus.share.TDebug.TraceCircularBuffer) {
                org.tritonus.share.TDebug.out("After read:");
                dumpInternalState();
                org.tritonus.share.TDebug.out("< completed. Read " + nLength + " bytes");
            }
            return nLength;
        }
    }
    ...
    public int write(byte[] abData, int nOffset, int nLength) {
        if (org.tritonus.share.TDebug.TraceCircularBuffer) {
            org.tritonus.share.TDebug.out(">TCircularBuffer.write(): called; nLength: " + nLength);
            dumpInternalState();
        }
        synchronized (this) {
            if (org.tritonus.share.TDebug.TraceCircularBuffer) {
                org.tritonus.share.TDebug.out("entered synchronized block.");
            }
            if (!m_bBlockingWrite) {
            ...
            if (org.tritonus.share.TDebug.TraceCircularBuffer) {
                org.tritonus.share.TDebug.out("After write:");
                dumpInternalState();
                org.tritonus.share.TDebug.out("< completed. Wrote " + nLength + " bytes");
            }
            return nLength;
        }
    }
    ...
    private void dumpInternalState() {
        org.tritonus.share.TDebug.out("m_lReadPos  = " + m_lReadPos + " ^= " + getReadPos());
        org.tritonus.share.TDebug.out("m_lWritePos = " + m_lWritePos + " ^= " + getWritePos());
        org.tritonus.share.TDebug.out("availableRead()  = " + availableRead());
